...
Mint.save = function() 
{
	...
	var path	= 'http://stephan-siegrist.ch/mint/?record&key=59584a54364b427854774f4e503234303039373433314a61554a';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(stephan-siegrist\.ch)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://stephan-siegrist.ch/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
...
Mint.TK.Durations = {
	...
	recalc : function() {
		var then = new Date();
		var img = new Image();
		var path = 'http://stephan-siegrist.ch/mint/pepper/tillkruess/durations/recalc.php?token=818135696&time=';
		path += Math.round((then.getTime() - Mint.TK.Durations.now.getTime()) / 1000);
		img.src = path.replace(/^https?:/, window.location.protocol);
	},
	onsave : function() {
		return '&token=818135696';
	}
};
